package io.grandino.kafka.example.javaspringkafkaexample;
...
public class JavaSpringKafkaSimpleConsumer {
    ...
    public static void main(String[] argv) throws  Exception{
        ...
        System.out.println("Reading messages fron Topic: "+ topicName);
        ...
        try(Consumer<String,String> consumer = new KafkaConsumer<String, String>(configProperties)){
            consumer.subscribe(asList(topicName));

            mainLoop:while(true){
                for(ConsumerRecord<String,String> message: consumer.poll(Duration.ofSeconds(1))){
                    System.out.println("Message received from particion=" + message.partition() + " with offset=" + message.offset());
